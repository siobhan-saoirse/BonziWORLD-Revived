(function (window, undefined) {
	...
	speak.play = function (text, args, onended, onstart, color) {
		...
		function startSource(source) {
			source.playbackRate.value = color
			//source.playbackRate.value = source.playbackRate.value - ((Math.random() - Math.random()) * 0.02)
			if (source.start) {
				source.start(0);
			} else {
				source.noteOn(0);
		...
		function playSound(streamBuffer) {
			...
			audioContext.decodeAudioData(streamBuffer, function (audioData) {
				...
				if (!isChrome && source.onended !== undefined) {
					...
					var duration = audioData.duration / source.playbackRate.value;
					// convert to msecs
